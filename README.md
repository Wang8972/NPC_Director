...
可选认知链已接入通用后端：配置模式、完成后模式提交、带来源的事件记忆、BM25/关联召回、
私有反思与修订、事件时钟衰减、受保护承诺，以及有预算和租约的后台维护。
设置 `NPC_DIRECTOR_COGNITION=true` 为新 episode 启用；默认保持 v2 兼容路径。
使用方法、结构图、迁移与回退见 [认知链说明](COGNITION.md)。
实际验证结果以 [认知链验证记录](VALIDATION_COGNITION.md) 为准，不能把 recorded 通过当作模型质量通过。
...
低成本模型的商业化对比见 [Qwen/Luna 评测](COMMERCIAL_MODEL_EVALUATION.md)：
当前 Luna 全链路为 72/102，Qwen Flash 在现有结构化调用协议下未通过兼容性试点。
