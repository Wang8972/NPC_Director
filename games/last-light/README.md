...
旧版介绍已原样保存为 [README 历史版本](Docs/README-legacy-2026-09-15.md)。
...
准备与 [资源锁清单](integration/assets.lock.json) 对应的 `LastLight-p4-assets.zip`，例如放在 `D:\Downloads`。该包没有上传到 Git；原开发机器保留于 `last-light/Builds/LastLight-p4-assets.zip`，需要另行取得或先导入 P4。
...
作用：同步 Git 源码和 SDK wheel，保留 P4 美术。受 P4 只读保护的代码快照、模型 `.meta`、生成资源和工程设置需先 checkout；脚本报覆盖冲突时不要强行替换。详细边界见 [Git/P4 说明](integration/README.md)。
...
存档归档：[LastLight-saves.zip](deliveries/2026-09-15/LastLight-saves.zip)。说明与校验：[存档清单](deliveries/2026-09-15/LastLight-saves.manifest.json)。包含实际游玩、开发测试和失败会话，以及对应 NPC 记忆、检查点与用量账本；不是全部通关的示范存档。归档不含模型凭据。
...
更多说明：[玩法与规则](Docs/RULES.md) · [模型与协议](Docs/DIRECTOR.md) · [人物与物品表现](Docs/PRESENTATION.md) · [资源许可](Docs/ASSET_LICENSES.md) · [已知问题](ISSUES.md)。
